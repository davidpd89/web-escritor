...
## 6. Correcciones (2026-09-08), tras revisión externa

- **Amazon — verificado en vivo**: las dos ediciones de Manecillas están públicamente visibles (Kindle 2,99 €, Tapa blanda 15,99 €, 266 págs. confirmado en la ficha pública), pero **siguen sin agruparse** bajo un mismo título — la ficha Kindle no muestra selector de formato hacia la edición en papel ni viceversa. Sigue siendo un pendiente real; si no se agrupan solas en los próximos días, hará falta abrir incidencia con Amazon/KDP o Monza.
- **Wikidata `Q139678851` — qualifier mal atribuido corregido**: la declaración `ocupación: escritor` llevaba colgada una **referencia** (no un qualifier, aunque el efecto práctico era el mismo: datos fuera de lugar) con el título "Samuel entre mundos" y el ISBN-13 979-13-87659-77-6 — ninguno de los dos describe la ocupación de escritor. Eliminada esa referencia por completo (verificado en el historial: −666 bytes). No hizo falta añadir esos datos en ningún otro sitio porque **ya estaban correctamente presentes** en `Q139915381` (título, vía P1476) y `Q139945987` (ISBN-13, vía P212) antes de esta corrección.
- **Wikidata — precisión de fecha corregida (17-dic-2025 → 2025)**: la sesión del 2026-09-07 fijó `P577 = 17 de diciembre de 2025` en `Q139915381` y `Q139945987`, tomando ese día exacto del runbook original (que a su vez lo atribuía a "la ficha pública de Goodreads"). Al revisar, ninguna fuente propia del proyecto respalda ese día concreto: el JSON-LD de `libros/samuel-entre-mundos/index.html` solo declara `"datePublished": "2025"` (año, sin mes ni día), y la PR de tracking **#429** (aún abierta) documenta explícitamente que hay discrepancias de fecha y de páginas sin resolver entre la autoridad del proyecto y los registros de distribución (Casa del Libro, Bookish, TodosTusLibros vía Libros Indie/DILVE). Afirmar un día exacto en Wikidata iba por delante de esa discrepancia todavía abierta. Corregido: ambos ítems ahora declaran `P577` con precisión de **año** (2025), que es la precisión que la propia web sí respalda. El número de páginas (422) **no se ha tocado**: a diferencia de la fecha, es el dato que la autoridad propia del proyecto declara de forma consistente en todo el sitio (no solo en Wikidata), así que mantenerlo alineado con esa autoridad no es sobre-afirmar — pero la discrepancia con los 412 de los retailers sigue abierta en #429 y no la resuelve esta corrección.
- **Google Preferred Sources / Cuaderno**: el primer despliegue del enlace rompió el gate de accesibilidad F.2 (resize-text 200%) en `/cuaderno/` porque el nuevo párrafo era un tercer hijo directo del grid de `.editorial-newsletter`. Corregido moviéndolo dentro del propio formulario, junto a la línea de política de privacidad ya existente.
- **Clarity**: ver commit y PR aparte (`fix/clarity-analytics-consent-denied-default-2026-09-08`) — `analytics_Storage` pasa a `denied` por defecto al no existir aviso de cookies en el sitio; `privacidad.html` actualizado para reflejar la base de interés legítimo real en vez de un consentimiento que nadie daba.
