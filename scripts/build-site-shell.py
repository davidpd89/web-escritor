...
REFERRER_START = "<!-- site-shell:referrer:start (generated by scripts/build-site-shell.py -- do not edit by hand) -->"
REFERRER_END = "<!-- site-shell:referrer:end -->"
...
# securityheaders.com flagged this as a missing header (2026-09-15 audit).
# Unlike X-Frame-Options/X-Content-Type-Options/Permissions-Policy (HTTP-header
# only, impossible on GitHub Pages -- see the CSP comment above on the same
# constraint), Referrer-Policy has a real <meta> equivalent that every current
# browser honors. "strict-origin-when-cross-origin" is the same value modern
# browsers already default to; declaring it explicitly turns an implicit
# default into a contract this site owns, rather than depending on browsers
# never changing that default.
...
def render_explore(nav: dict, by_id: dict[str, Entry], current_path: str, allow_newsletter: bool = True) -> str:
    ...
    subscribe_markup = (
        ...
        '      </div>\n'
        '      <p class="form-help">Al enviar tu email, aceptas la <a href="/privacidad.html">política de privacidad</a>.</p>\n'
...
EXISTING_REFERRER_RE = re.compile(r'<meta\s+name=["\']referrer["\']|<meta\s+http-equiv=["\']Referrer-Policy["\']', re.I)


def apply_referrer_policy(text: str, rel_path: str) -> str:
    """Inserta/renueva el Referrer-Policy publico del shell -- mismo patron que
    apply_public_csp(), pero sin hashes por pagina: el valor es constante."""
    marker_pattern = re.compile(re.escape(REFERRER_START) + r"[\s\S]*?" + re.escape(REFERRER_END))
    generated = f'<meta name="referrer" content="{REFERRER_POLICY}">'
    wrapped = with_markers(generated, REFERRER_START, REFERRER_END)

    if marker_pattern.search(text):
        return marker_pattern.sub(wrapped, text, count=1)
    if EXISTING_REFERRER_RE.search(text):
        return text
    charset_match = CHARSET_RE.search(text)
    if not charset_match:
        raise BuildError(f"{rel_path}: no se encontró <meta charset> para insertar el Referrer-Policy")
    insert_at = text.find("\n", charset_match.end())
    if insert_at == -1:
        insert_at = charset_match.end()
    return text[: insert_at + 1] + wrapped + "\n" + text[insert_at + 1 :]
...
def apply_shell(original: str, rel_path: str, nav: dict, by_id: dict[str, Entry], extras: dict) -> str:
    ...
    new_text = apply_referrer_policy(new_text, rel_path)
