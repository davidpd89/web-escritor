...
const viewports = [
  ...
  ['bio-901',901,900],['bio-900',900,900],['tablet-768',768,1024],['cards-701',701,900],['cards-700',700,900],
...
try{
  for(const [name,width,height] of viewports){
    ...
    try{
      ...
      assert.equal(await p.locator('.masthead-name-row__avatar').getAttribute('src'),'/assets/david-porto-avatar-masthead.webp',`${name}: asset de avatar eliminado del documento`);
      assert.equal(await p.locator('.masthead-name-row__avatar').evaluate(el=>getComputedStyle(el).display),'none',`${name}: avatar redundante reaparece en masthead`);
      ...
      const featuredBook=await style(p.locator('#libros .id-card--featured'));
      const awards=await style(p.locator('#premios .awards-ledger'));
      const quotes=await style(p.locator('#reseñas .quote-stream'));
      const mentions=await style(p.locator('#menciones .id-cards'));
      const writing=await style(p.locator('#escritura .id-cards'));
      const stats=await style(p.locator('.human-stats'));
      ...
      assert.equal(masthead.display,'block',`${name}: masthead vuelve a plantilla de dos columnas`);
      assert.equal(title.color,'rgb(29, 79, 150)',`${name}: H1 pierde azul canónico`);
      assert.equal(columns(bio.gridTemplateColumns),width>900?2:1,`${name}: seam 901/900 del dossier biográfico incorrecto`);
      assert.equal(columns(sectionHead.gridTemplateColumns),width>600?2:1,`${name}: seam 601/600 de cabecera de sección incorrecto`);
      assert.equal(columns(books.gridTemplateColumns),width>700?2:1,`${name}: seam 701/700 de obra incorrecto`);
      assert.equal(columns(mentions.gridTemplateColumns),width>700?2:1,`${name}: seam 701/700 de menciones incorrecto`);
      assert.equal(columns(writing.gridTemplateColumns),width>700?3:1,`${name}: seam 701/700 de voz incorrecto`);
      assert.ok(featuredBook.backgroundImage!=='none',`${name}: Manecillas pierde tratamiento dominante`);
      assert.equal(stats.borderLeftWidth,'0px',`${name}: estadísticas vuelven a panel/dashboard`);

      measurements.push({name,width,height,overflow,portraitMetrics,masthead,title,bio,portrait:portraitStyle,copy,aside,sectionHead,books,featuredBook,awards,quotes,mentions,writing,stats,activeContext,footer});
  ...
  for(const route of ['/premios.html','/eventos.html','/prensa.html']){
    ...
    try{
      ...
      const authorToken=await p.evaluate(()=>getComputedStyle(document.documentElement).getPropertyValue('--author-blue').trim());
      assert.equal(authorToken,'',`${route}: reglas exclusivas de Autor contaminan la página hermana`);
...
fs.writeFileSync(path.join(OUT,'autor-design-report.json'),JSON.stringify({route:'/autor.html',phase:'visual-system-contract',viewports:viewports.length,measurements,failures},null,2));
assert.deepEqual(failures,[],`Autor visual-system failures:\n${JSON.stringify(failures,null,2)}`);
console.log(`Autor visual system: PASS (${viewports.length} viewports + 3 isolation controls + no-JS + WCAG stress)`);
