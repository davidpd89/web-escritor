...
async function runResponsive(browser,report){
  for(const vp of viewports){report.responsive[vp.name]={};for(const def of pages){
    ...
    await page.goto(`${ORIGIN}${def.url}`,{waitUntil:'domcontentloaded'});await page.waitForTimeout(180);
...
async function runNoJs(browser,report){for(const def of pages){const c=await browser.newContext({javaScriptEnabled:false,viewport:{width:390,height:900}}),p=await c.newPage();await p.goto(`${ORIGIN}${def.url}`,{waitUntil:'domcontentloaded'});assert(await p.locator('h1').isVisible());assert(await p.locator('main a[href]').count()>=5,`${def.key}: orientación insuficiente sin JS`);await noOverflow(p,`${def.key}@nojs`);report.noJs[def.key]=true;await c.close();}}
async function runAccessibility(browser,report){for(const def of pages){const c=await browser.newContext({viewport:{width:390,height:900},reducedMotion:'reduce'}),p=await c.newPage();await p.goto(`${ORIGIN}${def.url}`,{waitUntil:'domcontentloaded'});assert.equal(await p.evaluate(()=>matchMedia('(prefers-reduced-motion: reduce)').matches),true);await checkFocus(p,def.key);
...
async function runFallback(browser,report){for(const def of pages){const c=await browser.newContext({viewport:{width:320,height:900}}),p=await c.newPage();await p.route('**/assets/fonts/**',r=>r.abort());await p.goto(`${ORIGIN}${def.url}`,{waitUntil:'domcontentloaded'});await p.waitForTimeout(120);assert(await p.locator('h1').isVisible());await noOverflow(p,`${def.key}@fallback`);report.fontFallback[def.key]=true;await c.close();}}
async function runCls(browser,report){for(const def of pages){const values=[];for(let run=0;run<3;run++){const c=await browser.newContext({viewport:{width:1440,height:1000}}),p=await c.newPage();await p.addInitScript(()=>{window.__cls=0;new PerformanceObserver(list=>{for(const e of list.getEntries())if(!e.hadRecentInput)window.__cls+=e.value}).observe({type:'layout-shift',buffered:true})});await p.goto(`${ORIGIN}${def.url}`,{waitUntil:'domcontentloaded'});await p.waitForTimeout(1200);const value=await p.evaluate(()=>window.__cls||0);values.push(value);assert(value<=0.10,`${def.key}: CLS ${value}`);await c.close();}report.cls[def.key]=values;}}
async function runScreens(browser,report){for(const def of pages){for(const vp of [{name:'1440',width:1440,height:1000},{name:'390',width:390,height:900}]){const c=await browser.newContext({viewport:{width:vp.width,height:vp.height}}),p=await c.newPage();await p.goto(`${ORIGIN}${def.url}`,{waitUntil:'domcontentloaded'});await p.waitForTimeout(250);const file=`${def.key}-${vp.name}.png`;await p.screenshot({path:path.join(OUT,file),fullPage:false});report.screenshots.push(file);await c.close();}}}
